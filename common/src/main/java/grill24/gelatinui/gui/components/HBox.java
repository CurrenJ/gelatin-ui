...
import grill24.gelatinui.gui.IUIElement;
import grill24.gelatinui.gui.IRenderContext;
import grill24.gelatinui.gui.UIContainer;
...
public class HBox extends UIContainer {
    ...
    protected void performLayout() {
        ...
        for (IUIElement child : children) {
            ...
            if (!scaleToFit && child instanceof grill24.gelatinui.gui.UIElement uiChild) {
        ...
        if (scaleToFit) {
            if (scaleFactor < 1.0f) {
                for (IUIElement child : children) {
                    if (!child.isVisible()) continue;
                    if (child instanceof grill24.gelatinui.gui.UIElement uiChild) {
        ...
        for (IUIElement child : children) {
            if (!child.isVisible()) continue;
            Vector2f childSize = child.getSize();
            float effectiveScale = scaleToFit ? scaleFactor : (child instanceof grill24.gelatinui.gui.UIElement uiChild ? uiChild.getCurrentScale() : 1.0f);
            ...
            if (animatePositions) {
                if (child instanceof grill24.gelatinui.gui.UIElement) {
                    ((grill24.gelatinui.gui.UIElement) child).setTargetPosition(targetPos, true);
                } else {
                    child.setPosition(targetPos);
                }
            } else {
                if (child instanceof grill24.gelatinui.gui.UIElement) {
                    grill24.gelatinui.gui.UIElement uiChild = (grill24.gelatinui.gui.UIElement) child;
